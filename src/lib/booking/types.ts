...
export type Settings = { enabled: boolean; nightly: number; cleaning: number; nightlyTwoGuests: number; cleaningTwoGuests: number; depositPercent: number; maxGuests: number; whatsapp: string; privacyUrl: string };
export type Rate = { id: string; name: string; start: string; end: string; nightly: number };
export type Coupon = { code: string; type: 'percent' | 'fixed'; value: number; start: string; end: string; limit: number; uses: number; active: boolean; accumulative?: boolean };
