...
export function Calendar({ data, start = '', end = '', onSelect, pending = [] }: { data: CalendarData; start?: string; end?: string; onSelect?: (day: string) => void; pending?: string[] }) {
  ...
  return <div className="min-w-0 max-w-full overflow-hidden rounded-2xl bg-white p-4 sm:p-6 text-gray-800 shadow-lg">
    ...
    <div className="grid min-w-0 grid-cols-7 text-center gap-1"><>{['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((day, i) => <span key={i} className="min-w-0 text-xs text-gray-500 py-2">{day}</span>)}</>
      ...
      {Array.from({ length }, (_, i) => {
        ...
        return <button key={day} type="button" disabled={!!onSelect && (day < today() || (blocked && !checkout))} onClick={() => onSelect?.(day)} aria-label={`${dateLabel(day)}: ${state}${checkout ? ', permitido para saída' : ''}`} aria-pressed={!!selected} title={`${dateLabel(day)}: ${state}`} className={`min-h-10 min-w-0 rounded-lg text-sm disabled:opacity-40 ${selected ? 'bg-green-800 text-white ring-2 ring-green-500' : state === 'Airbnb' ? 'bg-red-100 text-red-800' : state === 'Confirmada' ? 'bg-blue-100 text-blue-800' : pending.includes(day) ? 'bg-yellow-100 text-yellow-900' : 'bg-green-50 text-green-900'} ${onSelect ? 'hover:ring-2 hover:ring-green-500' : 'cursor-default'}`}>{i + 1}</button>;
      })}
    </div>
